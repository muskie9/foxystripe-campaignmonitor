...
class FSCampaignMonitorExtension extends ContentController {

    public function addIntegrations($data = null, $subscriptionData = null){

        if(isset($_POST['FoxyData'])) {$data = $_POST['FoxyData'];} else {return;}
        if(isset($_POST['FoxySubscriptionData'])) $subscriptionData = $_POST['FoxySubscriptionData'];

        foreach(CampaignMonitorIntegration::get() as $integration) {

            $CampaignMonitor_API_Key = $integration->APIKey; // Replace YOURAPIKEYHERE with your CampaignMonitor API Key.
            // How to find that?  Glad you asked, go here:
            // http://www.campaignmonitor.com/api/required/ to find this and the next few keys.

            $CampaignMonitor_Client_ID = $integration->ClientID;    // Replace CLIENTID with the Campaign Monitor Client ID.  See the above link.
            $CampaignMonitor_List_ID = $integration->ListID;        // Replace LISTID with the Campaign Monitor List ID.  See the above link.
...
////// END Configuration ///////
            ...
            $FoxyData = rc4crypt::decrypt($key, urldecode($data));
